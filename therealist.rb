...
puts "issue"
issue = gets.chomp
page = BASE_realist_URL + issue + "/" #"page" is the webpage where the specified issue is. (ex: issue one is in http://www.ep.tc/realist/01)
...
number = 1
pagina = page + "0" + number.to_s + ".jpg" # it defines a specific page of the magazine, starting with 1, it directs to the url 
#where the .jpg file where the page scan can be found. the format of the first 9 issues in the site, as well as the format of the first
# 9 pages of each issue is "00", so, when I'm converting it to string, I need to add a zero
...
pagina = page + number.to_s + ".jpg" #we don't need to add the "0" anymore, since all page number have two digits now.
	for number in 10...33 #still a place holder, need to find how to stop the loop when there are no more .jpgs to download.
		puts "Downloading page #{number}"
		open(pagina) {|f|
   File.open("#{ISSUE_DIR}/#{number}.jpg","wb") do |file|
     file.puts f.read
